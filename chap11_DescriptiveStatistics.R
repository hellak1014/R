...
mean(score) # 71.2
var(score) #693.2. 값이 다른 이유는 5로 나눈다고 생각하지만 사실은 나눌 값은 -1을 해야 하는게 맞기 때문에 4로 나눔. 이때 -1은 자유도라고 한다. 
...
#   ans) 조화 평균의 식 : 2xy / (x+y) = 2(100*60) / (100+60) = 75km/h
...
# 실습 데이터 셋 가져오기
data <- read.csv("c:/workspaces/Rwork/data/descriptive.csv", header = T)
View(data)
...
summary(data$level) # 명목척도와 함께 의미없음. na 갯수 13개. 115/99
...
pie(x1) #빈도수의 비율을 보는것. 의미가 있다. survey값의 빈도 비율을 보는것. 

# 2.4 비율척도 기술 통계량

# 생활비(cost) 변수 대상 요약 통계량 구하기
length(data$cost) # 297 \연산의 의미가 있다. 
summary(data$cost) # 요약통계량-의미있음(mean)-8.784, 중위수-3.000. 중위값과 평균치의 값이 많이 다르다. -> 이상이 있음. 

plot(data$cost) #연속함수 보는 그래프. 기준치와 이상치 (튀어나간 동그라미) 있음.

# 데이터 정제(이상치 제거)
data <- subset(data, data$cost >= 2 & data$cost <= 10) # 총점기준
data
summary(data$cost)#med 5.400, mean 5.354. 비슷한걸로 봐선 정규분포를 따를 가능성 多

plot(data$cost)
x <- data$cost #x=data$cost
mean(x) # 5.354032
...
# 생활비(cost) 변수 대상 대표값 구하기. 대표값의 의미가 있다.
mean(x)
median(x)
sort(x) # 데이터 오름차순
sort(x, decreasing=T) # 데이터 내림차순
...
names(x.df[19]) # [1] "5". 컬럼의 이름을 반환해줌. 


# (2) 산포도 구하기

# 생활비(cost) 변수 대상 산포도 구하기. 평균의 단점을 보완한 분산, 표준 편차등을 구하기. 
var(x) # 분산, [1] 1.296826
...
x <- table(data$cost2) #범주화 된 변수를 추가한다. (1/2/3)
barplot(x) #2에 짱많음. 2이상 4미만의 값이 많다.  
pie(x)
...
# 왜도 - 평균을 중심으로 기울어진 정도.
skewness(cost) # [1] -0.297234 => 오른쪽으로 기울어진 산. 


# 첨도 - 표준 정규 분포와 비교하여 얼마나 뽀족한가 측정 지표
kurtosis(cost) # [1] 2.674163 => 위로 뾰족하다. 
...
# 표준정규분포 곡선
x <- seq(0, 8, 0.1) #0~8까지 0.1단위로 곡선.
x
curve(dnorm(x, mean(cost), sd(cost)), col='red', add = T) # 우측으로 쏠린게 보인다. 


# attach() / detach() 함수로 기술 통계량 구하기
# - 기존 선언 변수와 컬럼의 이름이 중복시 기존 변수가 global 변수로 잡힘.
search()
attach(iris) #글로벌 변수로 만들어줌. attach : 데이터를 R 검색 경로에 추가하여 변수명으로 바로 접근할 수 있게 한다.
