...
par(mfrow=c(1,2))
log <- diff(AirPassengers) # 차분 수행. 현재시점 - 이전시점 자료
plot(log) # 평균 정상화
...
# 상관계수 : 변수와 변수사이의 상관성. 요인분석 및 회귀분석에서 상관분석을 전처리의 개념으로 사용한다.시계열에서는 일정시점의 데이터와 동일한 시차사이의 상관관계를 말한다. (자기 상관계수)
...
#내 현재값과 이전 값의 계수값 차이. 랜덤성과 규칙성을 평가. 

# 단계3 : 부분자기상관함수 시각화
pacf(na.omit(tsdata), main="부분자기상관함수", col="red")
#파란색 선 안에 값이 모두 있으면 의미가 있다. (유의미성을 가진다.) => 시간에 대한 의존성이 없고 랜덤하다 = 무작위성을 띈다. 
...
# 단계4 : 차분 시각화. 비정상의 데이터를 정상성으로 돌려놓는 작업. 
...
x11()
plot(tsdata, type="l", col='red') #비정상성 시계열 그래프. 평균이 일정하고 값이 일정하면 정상성. 이 그래프의 경우 진폭이 일정하지 않고 우상향하고 있으므로 비정상성 시계열 그래프이다. 
