...
# 고유값으로 요인 수 분석
en <- eigen(cor(subject)) # $values : 고유값, $vectors : 고유벡터 Correlation함수(cor)
...
result <- factanal(subject, factors = 2, rotation = "varimax") #2개 요인으로 한 결과. factanal함수로 넣기.
result # p-value is 0.0232  < 0.05 # 요인수가 부족. 너무 많이 압축이 되어있는 것. 연관성을 표현할 수 없다.
...
result
# Uniquenesses: 6개의 데이터 셋들의 변수의 유효성. 0.5이하면 유효.
# Loading : 유사성을 보여주는 데이터. 데이터 수치가 높을수록 유사하다. (factor3 s1-s2, factor2 s3-s4, factor1 s5-s6)
...
#요인 분석은 실제적 데이터 분석을 수행하기 전에 비슷한 항목을 묶는 전처리 분석법에 가깝다. 
...
cor(result$제품_친밀도, result$제품_적절성) # 0.4992086 -> 다소 높은 양의 상관관계 0.4이상이면 다소 높은 양의 상관관계. 
...
library(corrgram)
corrgram(result) # 색상 적용 - 동일 색상으로 그룹화 표시 진할수록 관계성이 높다. 
