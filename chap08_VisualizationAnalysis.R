...
histogram(~ gcsescore, data = Chem97) # gcsescore 변수를 대상으로 백분율 적용 히스토그램.빈도수 막대그래프.
...
histogram(~ gcsescore|factor(score), data = Chem97) # score 요인 단위. 가로는 점수. 세로는 점수별 백분율. 따라서 8점(각 화학 점수)를 맞은 사람들의 학력 평가에서 어떤 분포를 이루고 있는지에 대한 그래프.   
...
qplot(wt,mpg,data=mtcars) #차량이 무거울수록 연료는 더 많이 든다. 반비례.
