...
x <- seq(2.0, 4.5, 0.1) #2.0부터 4.5까지 0.1단위로 담기
x

curve(dnorm(x, mean=mean(iris$Sepal.Width), sd=sd(iris$Sepal.Width)), 
      col="blue", add=T)
#dnorm함수.x값을 넣어 정규분포곡선을 추가. 이때의 평균은 (iris$Sepal.Width)의 평균과 (iris$Sepal.Width)표준 편차의 유사성을 보기 위해서 한다. 
#정규분포 : 평균을 중심으로 좌우가 대칭형태를 이루는 것을  정규분포 곡선이라고 한다.
...
# pch 속성으로 그리기
plot(price, type="o", pch=5) # 빈 사각형 pch는 숫자에 따라 시각화되었을때 다양한 형태를 보여준다.
...
# 데이터프레임 생성
xy.df <- as.data.frame(table(x, y)) #테이블에 들어온 값을 데이터 프레임으로 강제 형변환. 
xy.df
...
plot(x, y, pch=15, col="blue",
     xlab = "x 벡터 원소",
     ylab = "y 벡터 원소",
     cex = 0.8 * xy.df$Freq) #cex : 0.8 크기로 리사이즈. Freq값을 곱한다. 
...
pairs(iris[,1:4]) # "Sepal.Length" "Sepal.Width"  "Petal.Length" "Petal.Width" 종데이터 를 제외한 나머지 4개를 선택 출력. 
#대각선을 기준으로 하나만 확인을 하면 분포도가 똑같다는 사실을 알 수 있음. 1대1 매칭을 통해 데이터 분포도를 보여준다. 
