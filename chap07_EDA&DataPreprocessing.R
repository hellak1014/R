...
# [색인(index)] 형식으로 변수 조회
dataset[2] # 두번째 컬럼(gender) - 출력형태:열 중. dataframe 형식. 
dataset[6] # price
dataset[3,] # 3행 전체
dataset[,3] # 3열 전체

# 두 개 이상의 [색인(index)] 형식으로 변수 조회. index외에도 select함수로도 읽어올수 있다. 
dataset[c("job", "price")]
...
dataset[-c(2)] # dataset[c(1, 3:7)]. -는 제외. 여기선 gender 제외.
...
sum(dataset$price) # NA. NA라는 공백이 하나라도 있으면 연산을 하는 함수들은 NA로 피드백한다. 

# 2.2 결측치 제거

# sum() 함수에서 제공되는 속성 이용
sum(dataset$price, na.rm = T) # 2362.9. na.rm : 데이터셋에서 결측치를 True로 셋팅한다. NA를 제외한 영역의 데이터만 처리. 

# 결측데이터 제거 함수 이용
price2 <- na.omit(dataset$price) #NA를 제거하고 처리. 
sum(price2) #2362.9
length(price2) # 270
...
# 3. 이상치(극단치) 처리 : 범위를 넘어가는 값 처리. subset 함수 사용. 
# 3.1 범주형 변수 이상치 처리
...
resident_gender
  ...
  barplot(resident_gender, beside = T, horiz = F, # beside = T : 그룹끼리 묶음. horiz = F : 막대를 세워그림.
        col=rainbow(5),
...
product_price <- dcast(pay_data,user_id ~ product_type, sum, na.rm=T) #userid별로 paydata를 product type 행으로 출력한다. 중복되는 타입의 물건 값은 더한다. 세로로 긴 데이터를 가로로 펼치는 셈. 
View(product_price)
...
library(plyr)
user_pay_data <- join(user_data, product_price, by='user_id') #구매자 아이디를 기준으로 두 테이블의 데이터를 join
View(user_pay_data)
...
choice1 #data의 index출력
data[choice1, 1] #data 값 출력. 여기서는 user id
