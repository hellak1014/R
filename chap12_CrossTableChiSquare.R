...
#----------------------------------------------------
#Chi^2 =  2.766951     d.f. =  2     p =  0.2507057  p-> 유의 확률. 유의 설정값. 사회과학분야에서는 0.05의 유의 설정값. 생명은 0.01. 
# 유의 확률 0.25가 0.05보다 크므로 귀무가설을 채택한다. 따라서 부모의 학력수준이 자녀의 대학진학 여부에 관련이 있다는 연구가설을 기각한다.
#혹은 Chi^2 =  2.76695(Chi-square contribution을 모두 더한값)와 d.f.(자유도) =  2를 바탕으로 카이제곱 분포표를 볼수 있다. 5.991 > 2.76695이므로 귀무가설을 채택한다. 
...
# 유의확률이 0.05보다 작다 -> 귀무가설을 기각한다. 스포츠음료의 선호도에 차이가 있다는 연구가설을 채택한다. 
# 확률표를 봐도 자유도 4에 사회과학 유의확률 0.05 기준 9.488이므로 카이제곱수가 더 크다 -> 귀무가설 기각. 
...
#method : 1. PPT만 2. 실습만 3. PPT와 실습 병행. 

table(data$method) # 교육방법
table(data$survey) # 만족도

# 전처리 - 결측치/ method와 survey 변수만 서브셋 생성
data <- subset(data, !is.na(survey), c(method, survey)) #na 없애기
data
