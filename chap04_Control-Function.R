...
x <- TRUE; y <- FALSE
xor(x, y) #Exclusive or. 두가지가 서로 다른 신호일때만 true를 반환함. 둘다 안들어오거나, 둘다 들어오면 False. 
x <- TRUE; y <- TRUE
xor(x, y)
...
#if(조건식){ 산술/비교/논리 연산자. 조건식에는 모든 문법이 다 올수 있으나 최종은 참거짓. boolean값이 나와야 한다. 
#    실행문1 <- 참
#}else{
#    실행문1 <- 거짓
#}
...
#java 3항 연산자. (boolean)? true:false ;
score <- c(78, 95, 85, 65) #한번에 여러개도 된다다
score
...
ifelse(q1 >= 3, sqrt(q1), q1) # 3보다 큰 경우 sqrt() 함수 적용. sqrt = 루트함수
...
for(n in i){ # 10회 반복. java의 enhanced for loop (향상된 for문)과 유사하다. 그래서 : 대신 in을 사용한다. 
  print(n*10)
  print(n)
  d[n] <- n*2
}
...
add <- f3(10, 30) #add를 하려고 보니까 f3이라는 함수를 호출하고 있다. 
add
...
# A 컬럼 요약통계량, 빈도수 구하기.
summary(test) # 요약통계량.
table(test$A) # A 변수 대상 빈도 수. (빈)도수분포표를 만드는 기본. 결과를 갯수로 나누면 확률분포표. 
max(test$A)   # 최대값
min(test$A)   # 최소값

length(test)  # 5. 열=column=데이터 분석상의 feature. 
class(test)
test3 <- c(1:9)
test3
length(test3) #벡터의 length 값은 9가 나옴. 이거... index값아니냐....?

# 각 컬럼 단위 요약통계량과 빈도 수 구하기.
data_pro <- function(x){

  for(idx in 1:length(x)){ #combine 함수 생략되어있다. 
...
var_sd <- function(x){
  var <- sum((x-mean(x))^2) / (length(x)-1) # 표본분산. 뒤에건 추정 통계를 통해 모집단을 추정하기 위함. 마지막 남은 것은 확실하게 알 수 있으므로 빼준다. 
  sd <- sqrt(var) #표준편차는 분산에 루트를 씌운값
  cat('표본분산:', var, '\n')
  cat('표본 표준편차:', sd, '\n')
}
...
mean(data) # NA. mean함수는 평균값.
mean(data, na.rm = T) # 10.77778. na.rm-> na로 되어있는 것들은 제거하고 계산해라. 
...
# 결측치 데이터 처리 함수. 
# 데이터의 형식이 data.frame일 경우, 한 구역이 NA라는 이유로 column 전체가 삭제 되는 것은 남아있는 데이터를 버리는 행위이고, 표본의 수가 줄어들면서 데이터의 신빙성이 낮아진다. 삭제하는 경우도 있으나, 0으로 만들거나 평균 값을 넣는 전처리를 시행한다. 
na <- function(x){
  # 1차 : NA 제거
  print(x)
...
seq(-2, 2, by = .2) # 0.2씩 증가. 벡터 데이터를 생성해주는 함수. (시작값, 종료값, 스텝텝)
vec <- 1:10
vec
...
# 단계3 : 이항분포(이산형) 난수 생성
n <- 20
rbinom(n, 1, prob = 1/2) #(범위, 발생할 갯수, 분위값)
rbinom(n, 2, 0.5)
rbinom(n, 10, 0.5)
n <- 1000
rbinom(n, 5, prob = 1/6)
...
set.seed(345) #seed값이 고정되면 발생 값도 동일하다. 
rnorm(5, mean=0, sd=1)
